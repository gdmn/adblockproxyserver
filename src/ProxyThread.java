...
class ProxyThread implements Runnable {
	...
	private static synchronized void decCounter() {
		counter--;
		Debug.println(2, "Thread counter = " + counter);
	}
	...
	public void run() {
		try {
			...
			if (pos > 0) {
				int endPos = pos+1;
				while (endPos<hostName.length() && ("0123456789".indexOf(hostName.charAt(endPos)) > -1) ) {
					endPos++;
				}
				try {
					hostPort = Integer.parseInt(hostName.substring(pos + 1, endPos));
				} catch (Exception e) {
				}
...

			{
				...
				int posGET = sRequest.indexOf("GET");
				if (posGET == 0) {
					int posGETend = sRequest.indexOf(" ", posGET + 4);
					String strGET = sRequest.substring(posGET + 4, posGETend);
...
//						if (strGET.contains(filter)) {
//							banned = true; break;
//						}
//					}
					String tmp = sRequest.substring(posGETend + 1, posGETend + 1 + 8);
					if (tmp.equals("HTTP/1.0")) {
						request[posGETend + 8] = (byte) '1'; // force HTTP/1.1
					}
					if (Filters.getInstance().match(strGET)) {
						banned = true;
					}
					if (banned) {
						Debug.println(1, "Blocked " + strGET + "");
						String errMsg = "HTTP/1.0 500\r\nContent Type: text/plain\r\n\r\n" +
								"Banned connection to the server: " + strGET + "\r\n\r\n";
						clientOut.write(errMsg.getBytes(), 0, errMsg.length());
					}
				}
			}
			...
			Debug.println(2, "Request from " + pSocket.getInetAddress().getHostAddress() +
			...
			if (request != null) {
				Debug.println(3, "REQUEST:\n" + (new String(request)));
			}
			if (response != null) {
				Debug.println(3, "RESPONSE:\n" + (new String(response)));
			}
			...
			Debug.println(1, "Error in ProxyThread: " + e);
		//e.printStackTrace();
		//System.err.println("");
		} finally {
			decCounter();
			...
			if (data != null) {
				...
				if ((data.toLowerCase().startsWith("http")) &&
					...
					try {
						...
						Debug.println(2, "Http responce code: " + responseCode);
					} catch (Exception e) {
						Debug.println(2, "Error parsing response code " + rcString);
					}
				}
			}
...
//                if (data.toLowerCase().contains("user-agent") ||
//                        data.toLowerCase().contains("accept") ||
//                        data.toLowerCase().contains("proxy") ||
//                        data.toLowerCase().contains("cache") ||
//                        data.toLowerCase().contains("pragma")) {
//                    continue;
//                }
                if (data.toLowerCase().contains("user-agent")) continue;
//                int poshttp10 = data.indexOf("HTTP/1.0");
//                    System.err.println(""+data);
//                if (poshttp10 > 0) {
//                    System.err.println(""+data);
//                    data = data.substring(0, poshttp10) + "HTTP/1.0";
//                }
				header.append(data + "\r\n");
			...
			if ((contentLength > 0) || (waitForDisconnect)) {
				try {
					...
					Debug.println(2, errMsg);
				//bs.write(errMsg.getBytes(), 0, errMsg.length());
				}
			}
		} catch (Exception e) {
			Debug.println(1, "Error getting HTTP data: " + e);
		}
	...
	private String readLine(InputStream in) {
		...
		try {
			...
			Debug.println(2, "Error getting header: " + e);
		}
